...
export function ImagePreviewGallery({
  ...
  const shouldShowUploadButton =
    isEditing && showUploadButton && !!onImageUpload && canUploadMore;
  ...
  if (!shouldShowUploadButton && images.length === 0 && !emptyState)
    return null;

  const uploadButton = shouldShowUploadButton ? (
  ...
  return (
    <>
      {layoutStyle === "grid" ? (
        <div className="absolute inset-0 flex flex-col bg-parchment/60">
          {uploadButton && (
          ...
          {images.length > 0 ? (
            <div className="flex-1 overflow-y-auto magic-scrollbar px-4 pb-1">
              <div className="grid min-h-full grid-cols-3 gap-3 md:gap-4 place-content-center">
                {images.map((img, idx) => (
                  <div
                    key={`${img.id}-${idx}`}
                    className="relative w-full aspect-square rounded-md overflow-hidden border-2 border-faded-gold group bg-rusty-copper/10 shadow-lg">
                    {img.loading ? (
        ...
        <div className="w-full">
          {uploadButton && <div className="mb-3">{uploadButton}</div>}
          {images.length > 0 ? (
            <div className="flex gap-3 overflow-x-auto py-2 magic-scrollbar w-full">
              {images.map((img, idx) => (
                <div
                  key={`${img.id}-${idx}`}
                  className="relative w-20 h-20 flex-shrink-0 rounded-md overflow-hidden border border-faded-gold group bg-rusty-copper/10">
                  {img.loading ? (
