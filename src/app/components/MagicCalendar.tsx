...
export function MagicCalendar({
  ...
  const highlightTodayOnly = displayMode === "todayOnly";
  ...
  return (
    <motion.div
      ...
      <div className="grid grid-cols-7 gap-1.5 relative z-10">
        ...
        {days.map((day) => {
          const matchingEntry = highlightTodayOnly
            ? undefined
            : entries.find((e) => isSameDay(new Date(e.date), day));
          const isToday = isSameDay(day, today);
          const isPicked = currentDate
            ? isSameDay(day, new Date(currentDate))
            : false;
          const isSelected = highlightTodayOnly ? isToday : isPicked;
            ...
            <button
              ...
              className={cn(
                ...
                isSelected
                  ...
                  : highlightTodayOnly && isPicked
                    ? "text-faded-gold border border-faded-gold/50 shadow-[0_0_10px_rgba(201,184,150,0.25)]"
                    : hasEntry
              ...
              {format(day, "d")}
              {hasEntry && !isSelected && !highlightTodayOnly && (
      ...
      {title && (
        <div className="mt-5 pt-3 border-t border-faded-gold/20 text-center relative z-10">
          <p className="text-xs text-faded-gold/60 italic font-serif">
            {title}
          </p>
        </div>
      )}
